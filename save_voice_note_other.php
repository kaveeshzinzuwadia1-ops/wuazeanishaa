...
// Directory to save other person's voice notes in the web debb folder
$uploadDir = 'C:\\Users\\DELL\\web debb\\other_voice_notes\\';
...
if (!is_dir($uploadDir)) {
    if (!mkdir($uploadDir, 0755, true)) {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Failed to create other_voice_notes directory'
        ]);
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['audio'])) {
    ...
    if (move_uploaded_file($file['tmp_name'], $filePath)) {
        ...
        echo json_encode([
            ...
            'path' => 'other_voice_notes/' . basename($filePath)
        ]);
    } else {
        http_response_code(500);
        echo json_encode([
